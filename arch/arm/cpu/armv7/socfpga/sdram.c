...
#define ADDRORDER1_INFO \
	"INFO: Changing address order to 1 (chip, bank, row, column)\n"
...
defined(CONFIG_HPS_SDR_CTRLCFG_CTRLCFG_NODMPINS)
	...
	/* Cyclone V SDRAM Controller register: ctrlcfg bitfield: addrorder */
	switch (CONFIG_HPS_SDR_CTRLCFG_CTRLCFG_ADDRORDER) {
	  default: /* intended rollover into case 0 which is the default ordering scheme*/
	  case 0: /* chip, row, bank, column Bank interleaved with no rank (chip select) interleaving */
		paddrorderinfo = ADDRORDER0_INFO; break;
	  case 1: /* chip, bank, row, column No interleaving */
		paddrorderinfo = ADDRORDER1_INFO; break;
	  case 2: /* row, chip, bank, column Bank interleaved with rank (chip select) interleaving */
		paddrorderinfo = ADDRORDER2_INFO;
	    if(CONFIG_HPS_SDR_CTRLCFG_DRAMADDRW_CSBITS < 2) {
	      /* chip select interleaving is only supported with more than one chip select */
	      /* set ordering scheme to default: chip, row, bank, column*/
	      addrorder = 0;
