...
int read_mac_from_eeprom(void)
{
    ...
    ret = spi_flash_probe_bus_cs(0, 0, CONFIG_SF_DEFAULT_SPEED, SPI_MODE_3, &dev);
    if (ret) {
        printf("Failed to initialize QSPI flash at 0:0 (error %d)\n", ret);
        return 1;
    } else {
        ret = spi_flash_read_dm(dev, CONFIG_SYS_QSPI_CUSTOM_AREA_OFFSET, CONFIG_SYS_QSPI_READ_BUF_SIZE, buf);
        if (ret)
...

{
    ...
    ret = spi_flash_read_dm(dev, CONFIG_SYS_QSPI_CUSTOM_AREA_OFFSET, spi_flash->erase_size, buf);
    ...
    ret = spi_flash_erase_dm(dev, CONFIG_SYS_QSPI_CUSTOM_AREA_OFFSET, spi_flash->erase_size);
    if (ret) {
        printf("QSPI Flash erase failed");
        goto error_free_mac_buf;
    }

    ret = spi_flash_write_dm(dev, CONFIG_SYS_QSPI_CUSTOM_AREA_OFFSET, spi_flash->erase_size, buf);
    if (ret) {
...
static int do_boardrev (struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    ret = spi_flash_read_dm(dev, CONFIG_SYS_QSPI_CUSTOM_AREA_OFFSET + CONFIG_SYS_QSPI_BOARD_REV_OFFSET,
            2, &ver);
...
static void add_FPGA_fileName(char* cmd, size_t maxlen)
{

#   define SOCFPGA_DEFAULT_TYPE "cv_se_a4"

    char *fpga_name_env = env_get("fpgatype");

    if (strncmp(fpga_name_env, SOCFPGA_DEFAULT_TYPE, sizeof(SOCFPGA_DEFAULT_TYPE)) == 0) {
        snprintf(cmd, maxlen, FPGA_FILE_NAME);
    }
    else
    {
        snprintf(cmd, maxlen, "m48_%s.img", fpga_name_env);
    }
}
...
int load_extra_images_from_partition(int partition)
{
    ...
    len = snprintf(cmdbuf, CMD_BUF_SIZE, "fatload mmc " __stringify(CONFIG_SYS_MMC_ENV_DEV) ":%d %x %s/",
        partition, FPGA_ADDR, imagepath);
    add_FPGA_fileName(&cmdbuf[len], CMD_BUF_SIZE - len);
...
int load_extra_images_from_tftp(void)
{
    ...
    len = snprintf(cmdbuf, CMD_BUF_SIZE, "tftp %x %s/", FPGA_ADDR, imagepath);
    add_FPGA_fileName(&cmdbuf[len], CMD_BUF_SIZE - len);
...
int start_itb_image(void)
{
	...
	if (do_get_device_type (NULL, 0, 0, NULL)) return -1;
     ...
     noffset = fit_conf_get_node((const void*) CONFIG_SYS_LOAD_ADDR, conf_name);
     fpga_name = fdt_stringlist_get((const void*) CONFIG_SYS_LOAD_ADDR, noffset, FIT_FPGA_PROP, 0,
    		 NULL);
     ...
     noffset = fit_image_get_node((const void*) CONFIG_SYS_LOAD_ADDR, fpga_name);

     if (fit_get_desc((const void*) CONFIG_SYS_LOAD_ADDR, noffset, &desc)) {
    	 return -1;
     } else {
    	 printf("FPGA type '%s'\n", desc);
     }

     if ( (memcmp(desc, data->device_type, DEVICE_TYPE_MNEMONIC) != 0) &&
            (memcmp(desc, DEVICE_TYPE_DEFAULT, DEVICE_TYPE_MNEMONIC) != 0)) {
         return -1;
     }

    snprintf(cmdbuf, CMD_BUF_SIZE, "bootm %s%s",
