 ...
 *  reboot of the board
 */
#define PERSISTEN_MEMORY_TEST_SIZE    (PM_RESERVED_MEM)
...
char* getPmMemoryRegionInDDR3(void)
{
    ...
    /* VxWorks PM region is at the End of DDR3 addr space     */
    /* gd->ram_size is set to the physical SDRAM size minus   */
    /* CONFIG_SYS_MEM_TOP_HIDE which is equal PM_RESERVED_MEM */
    address = CONFIG_SYS_SDRAM_BASE + gd->ram_size;
    if(  (address <  CONFIG_SYS_SDRAM_BASE)
        ||(address >= CONFIG_SYS_SDRAM_BASE + PHYS_SDRAM_SIZE))
    {
        return (char*)NULL;
    } else {
        return (char*)((uint32_t)address);
    }
}
...
void setEnvBootArgs(const char *netDeviceName)
{
    ...
    extra_opts_env = env_get("extraopts");

    if (extra_opts_env != NULL) {
        extraopts = extra_opts_env;
    }

    snprintf(cmdbuf, CMD_BUF_SIZE, "setenv bootargs \"%s(0,0)host:vxWorks h=%s e=%s:%s g=%s u=target pw=vxTarget %s\"",
    		netDeviceName, serverip_str, ipaddr_str, netmask_str, gwip_str, extraopts);
