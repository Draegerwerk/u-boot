...
#define DEFAULT_FDT_FILE_SSPPM "imx6q-2gb-pi-is8-ssppm.dtb"
#define DEFAULT_FDT_FILE_MOUNT "imx6q-2gb-pi-is8-mount.dtb"
...
#endif
    do_fixup_by_path(fdt, "/imx6/boardService", "preloader-version", splHandoverData->preloaderVersion, strlen((const char*) splHandoverData->preloaderVersion) + 1, 1);
    do_fixup_by_path(fdt, "/imx6/boardService", "bootloader-version", global_build_id, strlen(global_build_id) + 1, 1);
}
...
char* get_conf_name(void)
{
    ...
    if (check_name(conf_name_env, CMD_BUF_SIZE)) {
        ...
        else if (splHandoverData->boardVersion == VALENKA_IMX6Q_SSPPM) {
            conf_name = "ssppm";
        } else if (splHandoverData->boardVersion == VALENKA_IMX6Q_MOUNT) {
            conf_name = "mount";
        }
    }
    return conf_name;
...
const char * get_ftd_name(void)
{
    ...
    switch (splHandoverData->boardVersion) {
		...
		case VALENKA_IMX6Q_SSPPM:
            fdt_name = DEFAULT_FDT_FILE_SSPPM;
			break;
		case VALENKA_IMX6Q_MOUNT:
            fdt_name = DEFAULT_FDT_FILE_MOUNT;
			break;
        default:
            fdt_name = "unknown";
    }
...
{
    char *conf_name = NULL;
    image_header_t *image = (image_header_t *) CONFIG_SYS_LOAD_ADDR;

    conf_name = get_conf_name();
    snprintf(cmdbuf, CMD_BUF_SIZE, "bootm %x%s%s", image_get_ep(image),
...
int authenticate_and_start_fit_image(void)
{
    uint32_t offset;
    image_header_t *image = (image_header_t *) CONFIG_SYS_LOAD_ADDR;

    if (!image_check_magic(image)|| !image_check_hcrc(image)) {
        puts("ERROR:  image header wrong\n");
        return -1;
    }
     ...
     * CONFIG_CSF_SIZE = IVT_SIZE + CSF_PAD_SIZE + sizeof(image_header_t)
     */

    offset = image_get_size(image) - CONFIG_CSF_SIZE;
    if (imx_hab_authenticate_image(image_get_load(image),
        offset + IVT_SIZE +
        CSF_PAD_SIZE, offset)) {
...
{
    if (kernel_size == 0)
        return authenticate_and_start_fit_image();
    else
...
int display_error_message(void)
{
	...
	if (fdtdec_get_config_bool(gd->fdt_blob, "rotate_screen")) {
...
int dram_init(void)
{
    if (isValenka())
    {
        gd->ram_size = VALENKA_DRAM_SIZE0;
    }
    else
    {
...
int ft_board_setup(void *blob, struct bd_info *bd)
{
	...
	if (splHandoverData->boardVersion == VALENKA_IMX6Q_SSPPM) {
		ftd_fixup_ppm_eeprom(PPM_UNIT_SSPPM, blob);
	} else if (splHandoverData->boardVersion == VALENKA_IMX6Q_MOUNT) {
		ftd_fixup_ppm_eeprom(PPM_UNIT_MOUNT, blob);
	}
	return 0;
}
...
int board_late_init(void)
{
    ...
    if (splHandoverData->boardVersion == VALENKA_IMX6Q_SSPPM) {
        ...
        if (!env_get("extraopts"))
            env_set("extraopts", "s=/sd0:1/boot/ifconfigSsppm");
    } else if (splHandoverData->boardVersion == VALENKA_IMX6Q_MOUNT) {
        if (!env_get("panel"))
            env_set("panel", "hdmi-1280x800-32@60");
        if (!env_get("extraopts"))
            env_set("extraopts", "s=/sd0:1/boot/ifconfigMount");
    } else {
        if (!env_get("panel"))
        	env_set("panel", "640x480-24@60");
    }
...
int board_reset_post_test (int flags)
{
    ...
    if (isM48withUp2() && m48PmData->post_board_reset.result == M48_TS_NOT_RUN) {
